...
function SendMailTopDivergence(report){
    ...
    const msg = {
        ...
        subject: report.t,
        text: 'what!!',
        html: '<strong>'+"Top Divergence"+"<br>"+'Highest RSI: '+ report.max.r.toFixed(3)+' Price: '+report.max.p+'<br>'+'Current RSI: '+report.r.toFixed(3)+' Price '+report.p+'</strong>',
      };
    console.log(msg)
    //sgMail.send(msg);
    console.log('sent')
...
function RSIcomparison(){
    if(max.r>=70){
        ...
        if (RSI[168]>=max.r){
            ...
            // console.log(' ')
            report = {t:'Top divergence ',max,r: RSI[168],p:newdata.c[168]}
